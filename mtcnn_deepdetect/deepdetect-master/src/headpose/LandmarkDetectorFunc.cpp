...
{
	if (!clnf_model.detected_landmarks.empty() && clnf_model.params_global[0] != 0) //clnf_model.params_global
	{
		...
		//cout<<landmarks_2D.rows<<",col="<<landmarks_2D.cols<<endl; //136,1
		
		landmarks_2D = landmarks_2D.reshape(1, 2).t();
		//cout<<landmarks_2D.rows<<",col="<<landmarks_2D.cols<<endl;
		
		// 3D points
		cv::Mat_<double> landmarks_3D;
		clnf_model.pdm.CalcShape3D(landmarks_3D, clnf_model.params_local); //clnf_model.params_local
...
bool LandmarkDetector::DetectLandmarksInImage(const cv::Mat_<uchar> &grayscale_image, const cv::Rect_<double> bounding_box, CLNF& clnf_model, FaceModelParameters& params)
{
	...
	//cout<<params.multi_view<< ", params.multi_view 2222222222222"<<endl;
	...
	//cout<<clnf_model.hierarchical_models.size()<<", clnf_model.hierarchical_models.size() 33333"<<endl;
	...
	//cout<<"params_local:"<<clnf_model.params_local.rows<<", col="<<clnf_model.params_local.cols<<endl;
	//
	for(size_t hypothesis = 0; hypothesis < rotation_hypotheses.size(); ++hypothesis)
	{
		...
		//cout<<clnf_model.params_local<<endl;
		// calculate the local and global parameters from the generated 2D shape (mapping from the 2D to 3D because camera params are unknown)
