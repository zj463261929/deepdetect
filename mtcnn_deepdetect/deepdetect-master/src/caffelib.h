...
namespace dd
{
  ...
  template <class TInputConnectorStrategy, class TOutputConnectorStrategy, class TMLModel=CaffeModel>
	...
	int create_model_mtcnn(const bool &test=false); //2017-11-10 modify by zj
	 ...
	 */
	void init_mllib(const APIData &ad, int nIndex=0); //2017-11-10 modify by zj mtcnn
	...
	int predict_mtcnn(const APIData &ad, APIData &out);
	int predict_mtcnn_emotion_atention(const APIData &ad,APIData &out); ////2017-11-17 modify by zj mtcnn+emotion+attention
	//TODO: status ?
	...
	public:
		...
		caffe::Net<float> *_net1p = nullptr; //2017-11-10 modify by zj
		caffe::Net<float> *_net2r = nullptr;
		caffe::Net<float> *_net3o = nullptr;
