...
namespace dd
{
  ...
  void CaffeLib<TInputConnectorStrategy,TOutputConnectorStrategy,TMLModel>::set_gpuid(const APIData &ad)
  {
	...
	if (ad.has("gpuid"))
	{
		...
		for (auto i: _gpuid)//for (int i=0; i<_gpuid.size(); i++)		
													  ...
													  caffe::NetParameter &dnet_param)
  {
	...
	template <class TInputConnectorStrategy, class TOutputConnectorStrategy, class TMLModel>
	int CaffeLib<TInputConnectorStrategy,TOutputConnectorStrategy,TMLModel>::create_model_mtcnn(const bool &test)
	{
		LOG(INFO)<<"caffelib.cc::create_model_mtcnn !!!";
		if(!test)
		{
			return 1;
		}

		if (!this->_mlmodel._def1.empty() && !this->_mlmodel._weights1.empty()
			&& !this->_mlmodel._def2.empty() && !this->_mlmodel._weights2.empty()
			&& !this->_mlmodel._def3.empty() && !this->_mlmodel._weights3.empty())
		{
			delete _net1p;
			_net1p = nullptr;
			delete _net2r;
			...
			try
			{
				_net1p = new Net<float>(this->_mlmodel._def1,caffe::TEST);
				_net2r = new Net<float>(this->_mlmodel._def2,caffe::TEST);
				_net3o = new Net<float>(this->_mlmodel._def3,caffe::TEST);
			}
			catch (std::exception &e)
			{
			...
			LOG(INFO) << "Using pre-trained weights1(mtcnn) from " << this->_mlmodel._weights1;
			LOG(INFO) << "Using pre-trained weights2(mtcnn) from " << this->_mlmodel._weights2;
			LOG(INFO) << "Using pre-trained weights3(mtcnn) from " << this->_mlmodel._weights3;
			LOG(INFO) << "Using det1.prototxt(mtcnn) from " << this->_mlmodel._def1;
			LOG(INFO) << "Using det2.prototxt(mtcnn) from " << this->_mlmodel._def2;
			LOG(INFO) << "Using det3.prototxt(mtcnn) from " << this->_mlmodel._def3;
			try
			{
				_net1p->CopyTrainedLayersFrom(this->_mlmodel._weights1);
				_net2r->CopyTrainedLayersFrom(this->_mlmodel._weights2);
				_net3o->CopyTrainedLayersFrom(this->_mlmodel._weights3);
			}
			catch (std::exception &e)
			{
		...
		else if (this->_mlmodel._def1.empty() 
				|| this->_mlmodel._def2.empty() 
				|| this->_mlmodel._def3.empty())
	...
	{
		LOG(INFO)<<"caffelib.cc::create_model !!!";
		...
		if (!this->_mlmodel._def.empty() && !this->_mlmodel._weights.empty())
		{
			...
			LOG(INFO) << "Using *.prototxt from " << this->_mlmodel._def;
			try
			{
	...
	template <class TInputConnectorStrategy, class TOutputConnectorStrategy, class TMLModel>
	void CaffeLib<TInputConnectorStrategy,TOutputConnectorStrategy,TMLModel>::init_mllib(const APIData &ad, int nIndex)
	{
		if (ad.has("gpu"))
		...
		else if (1==nIndex) // model template instantiation is defered until training call
		{
			LOG(INFO)<<"caffelib.cc init_mllib mtcnn !!!";
			create_model_mtcnn(true);
			
		}
		else if (2==nIndex)
		{
			LOG(INFO)<<"caffelib.cc init_mllib mtcnn+emotion+atention !!!"; //2017-11-17 modify by zj
			create_model_mtcnn(true);
			create_model(true);
		}
		else
										 ...
										 APIData &out)
  {
	...
	template <class TInputConnectorStrategy, class TOutputConnectorStrategy, class TMLModel>
	int CaffeLib<TInputConnectorStrategy,TOutputConnectorStrategy,TMLModel>::predict_mtcnn_emotion_atention(const APIData &ad,APIData &out)
	{
		LOG(INFO)<<"caffelib.cc::predict_mtcnn+emotion+atention !!!"<<std::endl;
		std::lock_guard<std::mutex> lock(_net_mutex); 
		
		if (!_net1p || _net1p->phase() == caffe::TRAIN
			|| !_net2r || _net2r->phase() == caffe::TRAIN
			|| !_net3o || _net3o->phase() == caffe::TRAIN)
		{
			int cm = create_model_mtcnn(true);
			if (cm != 0)
				LOG(ERROR) << "Error creating model for prediction";
			if (cm == 1)
				throw MLLibInternalException("mtcnn no model in " + this->_mlmodel._repo + " for initializing the net");
			else if (cm == 2)
				throw MLLibBadParamException("mtcnn no deploy file in " + this->_mlmodel._repo + " for initializing the net");
		}
		...
		if ( ad.has("mtcnn") )
		{
			if ( ad.getobj("mtcnn").has("minSize") )
				minSize = ad.getobj("mtcnn").get("minSize").get<int>();
			if ( ad.getobj("mtcnn").has("factor") )
				factor = ad.getobj("mtcnn").get("factor").get<double>();
			if ( ad.getobj("mtcnn").has("threshold") )
			{
				std::vector<double> vdData= ad.getobj("mtcnn").get("threshold").get<std::vector<double>>();
				for (int i=0; i<std::min(3,int(vdData.size())); i++)
				{
					threshold[i] = vdData[i];
				}
			}
		}
		LOG(INFO)<<"minSize="<<minSize<<",factor="<<factor<<",threshold="<<threshold[0]<<","<<threshold[1]<<","<<threshold[2]<<std::endl;
		...
		{
			if ( ad_output.get("bbox").get<bool>())
			{
				bbox = true;
			}
		}
		...
		// process mean
		float *mean = nullptr;
		std::string meanfname = "mean.binaryproto";
		bool has_mean_file = this->_mlmodel._has_mean_file;
		caffe::Blob<float> data_mean; // mean binary image if available.
		std::string meanfullname = this->_mlmodel._repo + "/" + meanfname;
		//LOG(INFO)<<this->_mlmodel._corresp<<", this->_mlmodel._corresp";
		//LOG(INFO)<<meanfullname<<", meanfullname";
		if (data_mean.count() == 0 && has_mean_file)
		{
		...
		std::vector<vector<FaceInfo>> vtotalfaceInfo;
		std::vector<std::vector<double>> vvdEmotionProbs;
		std::vector<std::vector<std::vector<double>>> vvvdEmotionProbs; //
        std::vector<std::string> vsEmotionCats;
        std::vector<std::vector<std::string>> vvsEmotionCats;
        std::vector<std::vector<std::vector<std::string>>> vvvsEmotionCats;
		...
		if (ad.has("service") )
		{
			sServiceName = ad.get("service").get<std::string>();
		}
		
		std::string sCSVName = "emotion.csv";
		bool bCSV = false;
		if ( ad.has("csv") )
		{
			sCSVName = ad.get("csv").get<std::string>();
			bCSV = true;
			
			std::ifstream iFile(sCSVName);  
			string  readStr((std::istreambuf_iterator<char>(iFile)),  std::istreambuf_iterator<char>());
			if (readStr.size() < 1)
			{
				std::ofstream oFile1;   
				//打开要输出的文件   
				oFile1.open(sCSVName, ios::out | ios::app); 
				oFile1<<"ImageName"<<","<<"ServiceName"<<","<<"Angry"<<","<<"Disgust"<<","<<"Fear"<<","<<"Happy"<<","<<"Neutral"<<","<<"Sad"<<","<<"Surprise"<<","<<","<<"Angry"<<","<<"Disgust"<<","<<"Fear"<<","<<"Happy"<<","<<"Neutral"<<","<<"Sad"<<","<<"Surprise"<<","<<","<<"Angry"<<","<<"Disgust"<<","<<"Fear"<<","<<"Happy"<<","<<"Neutral"<<","<<"Sad"<<","<<"Surprise"<<std::endl;
				oFile1.close();   
			}
		}
		...
		std::ofstream oFile;   
		//打开要输出的文件   
		oFile.open(sCSVName, ios::out | ios::app); 
		try
		{
			inputc.transform(cad);
			MTCNN detector;
			...
			char buffer[256]; //2017-11-10 modify by zj
			for (size_t nIndexImage=0; nIndexImage<inputc.vmImages.size(); nIndexImage++)
			{
				faceInfo.clear();
				vvdEmotionProbs.clear();///
				//vsEmotionCats.clear();
				cv::Mat image = inputc.vmImages[nIndexImage];

				// mtcnn
				clock_t t1 = clock();
				LOG(INFO) <<"Detect "<<image.rows<<"X"<<image.cols;
				detector.Detect(image,faceInfo,minSize,threshold, factor, _net1p, _net2r, _net3o);/////////
				LOG(INFO)<<" detect time: " << (clock() - t1)*1.0/1000;
				LOG(INFO)<<"faceInfo.size(): "<<faceInfo.size();
				vtotalfaceInfo.push_back(faceInfo);
				
				//emotion
				if ( faceInfo.size()>0 )
				{
					detector.Emotion(image, faceInfo, vvdEmotionProbs, _net, mean);
					
					if (vvdEmotionProbs.size()>0 && 0 == vsEmotionCats.size()) //get cats
					{
						for (size_t i=0;i<vvdEmotionProbs[0].size();i++) //vvdEmotionProbs[0].size()==nclasses
						{
							vsEmotionCats.push_back(this->_mlmodel.get_hcorresp(i));
						}
					}
				}
                vvvsEmotionCats.push_back(vvsEmotionCats);
				vvvdEmotionProbs.push_back(vvdEmotionProbs);
				/*
                //show
				for (size_t i=0; i<vvdEmotionProbs.size(); i++)
				{
					LOG(INFO)<<" ";
					int n = std::min(vvdEmotionProbs[i].size(),vsEmotionCats.size());
					for (int j=0; j<n; j++)
					{
						LOG(INFO)<<"face ID="<<i<<","<<vsEmotionCats[j]<<":"<<vvdEmotionProbs[i][j];
					}
				}
				//
                */
                
				//crop face: calculate head pose
				for(size_t i = 0;i<faceInfo.size();i++)
				{
					int x1 = int(faceInfo[i].bbox.x1);
					int x2 = int(faceInfo[i].bbox.x2);
					int y1 = int(faceInfo[i].bbox.y1);
					int y2 = int(faceInfo[i].bbox.y2);
					int w = x2 - x1;
					int h = y2 - y1;
					/*x1 = x1 - w*0.1;
					x2 = x2 + w*0.1;
					y1 = y1 - h*0.2;
					y2 = y2 + h*0.2;*/
					cv::Mat faceImg = image(cv::Range(x1,x2),cv::Range(y1,y2));
					cv::Vec3f pose;
					cHeadPose.getHeadPose(faceImg, pose);
					LOG(INFO)<<pose[0]<<","<<pose[1]<<","<<pose[2];
					sprintf( buffer, "face_%d.jpg", (int)i);
					cv::imwrite(buffer, faceImg);
				}
                ...
                //zj
				sprintf( buffer, "oNet_%d.jpg", (int)nIndexImage);
				...
				if( faceInfo.size() > 0 && Iswrite )
				{
					int nflag = 0;
					std::string img_uri = inputc._ids.at(nIndexImage);//only one
					int npoint =img_uri.rfind(".");
					if(npoint>0)
					{
						std::string strsuffix(img_uri.substr(npoint+1));
						if( strsuffix == "jpg" || strsuffix == "bmp" || strsuffix == "png" )
						{
							int num = img_uri.rfind("/");
							if(num<(int)img_uri.length() && num>0)
							{  
								string firstname(img_uri.substr(num+1));
								string imagename = "/home/ligang/deepdetect/mtcnn/result/"+firstname;
								sImageName = firstname;
								//sprintf( buffer, "pNet_%d.jpg", nIndexImage);
				...
				{
					if (faceInfo.size() > 0)
						oFile << sImageName << "," << sServiceName<< ",";
					for(size_t i=0; i<faceInfo.size(); i++)
					{ 
						for(size_t j=0; j<vvvdEmotionProbs[nIndexImage][i].size(); j++)
						{
							//vvvsEmotionCats[nIndexImage][i]
							sprintf( buffer, "%.3f", vvvdEmotionProbs[nIndexImage][i][j]);
							oFile << buffer << ",";  
						}
						oFile <<",";
					}
				...
				//////////////////
				LOG(INFO)<<"faceInfo.size = "<<faceInfo.size();
				LOG(INFO)<<"vtotalfaceInfo.size = "<<vtotalfaceInfo.size();
			}//2017-11-10 modify by zj
			oFile.close();   
		}
		catch (std::exception &e)
		{
		...
		int batch_size = inputc.vmImages.size();
		if(vtotalfaceInfo.size() != inputc.vmImages.size())
		{
			LOG(ERROR) << "The number of results "<<vtotalfaceInfo.size()<<" and the number of pictures "<<batch_size<<" are different !";
			return 0;
		}
        //#####################################################
        //2017-11-20 modify by lg
		//result 

        int sizevvvdProbs = vvvdEmotionProbs.size();
        if( batch_size != sizevvvdProbs)
        {
            LOG(ERROR) << "The number of Images "<<batch_size<<" and the number of Emotion "<<sizevvvdProbs<<" are different !";
			return 0;
        }
		...
		if (bbox) // in-image object detection
		{
			...
			for (int j=0;j<batch_size;j++)
			{
                ...
                std::string stremotioncat;
				APIData rad;
                
				std::string uri = inputc._ids.at(idoffset+j);
				for (size_t k = 0;k<vtotalfaceInfo[j].size();k++)
				{
                    if( vtotalfaceInfo[j].size() != vvvdEmotionProbs[j].size())
                    {
                        LOG(ERROR) << "The number of "<<j<<" pictures face "<<vtotalfaceInfo[j].size()<< "and the number of emotion "<<vvvdEmotionProbs[j].size()<<" are different !";
                        return 0;
                    }
					if (vtotalfaceInfo[j][k].bbox.score < confidence_threshold)
					{
						continue;
					}
					probs.push_back(vtotalfaceInfo[j][k].bbox.score);
					//cats.push_back("face");
					APIData ad_bbox;
					ad_bbox.add("xmin",vtotalfaceInfo[j][k].bbox.y1);//2017-11-17 modify by lg
                    ad_bbox.add("ymax",vtotalfaceInfo[j][k].bbox.x2);
                    ad_bbox.add("xmax",vtotalfaceInfo[j][k].bbox.y2);
                    ad_bbox.add("ymin",vtotalfaceInfo[j][k].bbox.x1);
                    //
                    
                    int sizeCat = vsEmotionCats.size();
                    if( sizeCat != _nclasses )
                    {
                        LOG(ERROR) << "The number of cat "<<vsEmotionCats.size()<<" and the number of nclass "<<_nclasses<<" are different !";
                        return 0;
                    }
        
                    if( (int)vvvdEmotionProbs[j][k].size() != sizeCat )
                    {
                        LOG(ERROR) << "The number of "<<j<<" pictures "<<k<<" face EmotionProbs"<<vvvdEmotionProbs[j][k].size()<< "and the number of vsEmotionCats "<<sizeCat<<" are different !";
                        return 0;
                    }
                    
                    stremotioncat = "";
                    for ( int ncat = 0;ncat<sizeCat;ncat++ )
                    {
                        ad_bbox.add(vsEmotionCats[ncat],vvvdEmotionProbs[j][k][ncat]);
                        //LOG(INFO)<<"EmotionProbs - "<<vsEmotionCats[ncat]<<" = "<<vvvdEmotionProbs[j][k][ncat];
                        if( ncat > 0 )
                        {
                            stremotioncat += ",";
                        }
                        stremotioncat += vsEmotionCats[ncat];
                        
                    }
					bboxes.push_back(ad_bbox);
                    cats.push_back(stremotioncat);
				}
				rad.add("uri",uri);
		...
		{
			LOG(ERROR) << "inputparam have no bbox";
			return 0;
		}		 
		if (!inputc._segmentation)
		{
		...
		LOG(INFO)<<"caffelib.cc::predict_mtcnn_emotion_atention return.";
	...
	template <class TInputConnectorStrategy, class TOutputConnectorStrategy, class TMLModel>
	int CaffeLib<TInputConnectorStrategy,TOutputConnectorStrategy,TMLModel>::predict_mtcnn(const APIData &ad,APIData &out)
	{
		LOG(INFO)<<"caffelib.cc::predict_mtcnn !!!"<<std::endl;
		std::lock_guard<std::mutex> lock(_net_mutex); 
		...
		try
		{
			...
			MTCNN detector;
			//char buffer[256]; //2017-11-10 modify by zj
			for (size_t i=0; i<inputc.vmImages.size(); i++)
			{
				faceInfo.clear();
				cv::Mat image = inputc.vmImages[i];
				//sprintf( buffer, "src_%d.jpg", i);
				//cv::imwrite(buffer, inputc.vmImages[i]);
				...
				//write
				bool Iswrite = false;//true;
				if( faceInfo.size() > 0 && Iswrite )
				{
					int nflag = 0;
					std::string img_uri = inputc._ids.at(i);//only one
					unsigned int npoint =img_uri.rfind(".");
					if(npoint != img_uri.npos)
					{
						std::string strsuffix(img_uri.substr(npoint+1));
						if( strsuffix == "jpg" || strsuffix == "bmp" || strsuffix == "png" )
						{
		...
		{
			tout.add_results(vrad);
			LOG(INFO)<<"tout.add_results(vrad);";
		}
		...
		LOG(INFO)<<"caffelib.cc::predict_mtcnn return.";
		
		return 0;
	}
	
	template <class TInputConnectorStrategy, class TOutputConnectorStrategy, class TMLModel>
	int CaffeLib<TInputConnectorStrategy,TOutputConnectorStrategy,TMLModel>::predict(const APIData &ad,APIData &out)
	{
		std::lock_guard<std::mutex> lock(_net_mutex); // no concurrent calls since the net is not re-instantiated
		LOG(INFO)<<"caffelib.cc::predict !!! "<<std::endl;
		// check for net
		if (!_net || _net->phase() == caffe::TRAIN)
		{
			int cm = create_model(true);
					...
					for (int j=0;j<batch_size;j++)
					{
						...
						for (int i=0;i<nclasses;i++)
						{
							...
							if (prob < confidence_threshold)
								continue;
							LOG(INFO)<<"i="<<i<<", prob="<<prob<<", 999999999999999";
