...
namespace dd
{
	CaffeModel::CaffeModel(const APIData &ad, int nIndex):MLModel()
	{
		if (ad.has("templates"))
		...
		{
			if ( 1==nIndex ) ////2017-11-10 modify by zj mtcnn
			{
				if (read_from_repository_mtcnn(ad.get("repository").get<std::string>()))
					throw MLLibBadParamException("error reading or listing Caffe models(mtcnn) in repository " + _repo);
			}
			else if( 2==nIndex )//2017-11-10 modify by zj mtcnn+emotion+atention
			{
				if (read_from_repository_mtcnn(ad.get("repository").get<std::string>()) || read_from_repository(ad.get("repository").get<std::string>()))
				{
					throw MLLibBadParamException("error reading or listing Caffe models(mtcnn) in repository " + _repo);
				}
			...
			sModelPath = ad.get("repository").get<std::string>();
		}
		
		read_corresp_file(); //mlmodel.h
	}
	
	int CaffeModel::read_from_repository_mtcnn(const std::string &repo)
	{
		LOG(INFO)<<"read mtcnn model !!!"<<std::endl;
		static std::string deploy1 = "det1.prototxt";
		static std::string deploy2 = "det2.prototxt";
		static std::string deploy3 = "det3.prototxt";
		static std::string train = ".prototxt";
		static std::string weights1 = "det1.caffemodel";
		static std::string weights2 = "det2.caffemodel";
		static std::string weights3 = "det3.caffemodel";
		...
		std::string deployf1,deployf2,deployf3,trainf,weightsf1,weightsf2,weightsf3,correspf,solverf,sstatef;
		...
		while(hit!=lfiles.end())
		{
			...
			else if ((*hit).find(weights1)!=std::string::npos)
			{
				// stat file to pick the latest one
			...
			else if ((*hit).find(weights2)!=std::string::npos)
			{
				// stat file to pick the latest one
				long int wt = fileops::file_last_modif((*hit));
				if (wt > weight_t)
			...
			else if ((*hit).find(weights3)!=std::string::npos)
			{
				// stat file to pick the latest one
				long int wt = fileops::file_last_modif((*hit));
				if (wt > weight_t)
			...
			else if ((*hit).find(deploy1)!=std::string::npos)
				deployf1 = (*hit);
			else if ((*hit).find(deploy2)!=std::string::npos)
				deployf2 = (*hit);
			else if ((*hit).find(deploy3)!=std::string::npos)
				deployf3 = (*hit);
	...
	{
		LOG(INFO)<<"read model !!!"<<std::endl;
		static std::string deploy = "deploy.prototxt";
		static std::string train = ".prototxt";
		static std::string weights = ".caffemodel"; //2017-11-17 modify by zj
		static std::string sstate = ".solverstate"; //
		static std::string corresp = "corresp";
		static std::string solver = "_solver.prototxt";
		static std::string meanf = "mean.binaryproto";
