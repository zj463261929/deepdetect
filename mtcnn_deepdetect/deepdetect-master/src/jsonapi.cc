...
namespace dd
{
				  ...
				  const uint32_t &dd_code, const std::string &dd_msg) const
  {
	...
	JDoc JsonAPI::dd_ng_700() const
	{
		JDoc jd;
		jd.SetObject();
		render_status(jd,700,"more pictures");
		return jd;
	}
				   ...
				   const std::string &jstr)
	{
		...
		try
		{
			...
			if (d.HasMember("description"))
				description = d["description"].GetString(); //description = image classification service
		...
		try
		{
			if (mllib == "caffe")
			{
				...
				CaffeModel cmodel(ad_model, nIndex); //2017-11-10 modify by zj
				if (type == "supervised")
				{
					if (input == "image")
					{
				   ...
				   const std::string &jstr)
  {
	...
	{
		//LOG(INFO)<<"JsonAPI::service_predict 11111111111"<<std::endl;
		rapidjson::Document d;
		d.Parse(jstr.c_str());
		if (d.HasParseError())
		{
		...
		try
		{
			//LOG(INFO)<<"JsonAPI::service_predict 11111111111 start"<<std::endl;
			std::vector<std::string> vsImage;
			vsImage = ad_data.get("data").get<std::vector<std::string>>(); 
			/*
			if (1==vsImage.size())
			{
				this->predict(ad_data,sname,out); // we ignore returned status, stored in out data object
			}
			else if (vsImage.size()>1)
			{
				JDoc jpred = dd_ng_700();
				LOG(ERROR)<<"Input cannot be more than one picture!!!!!!!!"<<std::endl;
				return jpred;
			}
			*/	
			this->predict(ad_data,sname,out); // we ignore returned status, stored in out data object
			//LOG(INFO)<<"JsonAPI::service_predict 11111111111 end"<<std::endl;
		}
		catch (InputConnectorBadParamException &e)
		{
			return dd_service_input_bad_request_1005();
