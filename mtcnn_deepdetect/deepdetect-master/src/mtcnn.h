...
class MTCNN {
	public:
		//void create_model(const string& proto_model_dir, bool isCPU=true);
		void Detect(const cv::Mat& img, std::vector<FaceInfo> &faceInfo, int minSize, double* threshold, double factor, caffe::Net<float> *_net1p, caffe::Net<float> *_net2r, caffe::Net<float> *_net3o);
	public: //emotion
		int Emotion(const cv::Mat& image, std::vector<FaceInfo>& faceInfo, std::vector<std::vector<double>>& vvdEmotionProbs,caffe::Net<float> *_net, const float *mean);
 
	private: //emotion
		int crop_emotion(const cv::Mat& image, const std::vector<FaceInfo>& faceInfo, std::vector<cv::Mat> &vmImages);
		int predict_emotion(const cv::Mat &image, caffe::Net<float> *_net, const float *mean, std::vector<double>& vdEmotionProbs);
	...
	private:
		...
		void GenerateBoundingBox(const boost::shared_ptr<Blob<float> > confidence,const boost::shared_ptr<Blob<float> > reg,
			float scale,float thresh,int image_width,int image_height);
		...
		caffe::Net<float> *net, double thresh, char netName);
...
void MTCNN::GenerateBoundingBox(const boost::shared_ptr<Blob<float> > confidence,const boost::shared_ptr<Blob<float> > reg,
	  float scale,float thresh,int image_width,int image_height)
{
	int stride = 2;
	int cellSize = 12;

	int curr_feature_map_w_ = std::ceil((image_width - cellSize)*1.0/stride)+1; //(288-12)/2 = 138
	int curr_feature_map_h_ = std::ceil((image_height - cellSize)*1.0/stride)+1;//(384-12/2) = 186

	//std::cout << "Feature_map_size:"<< curr_feature_map_w_ <<" "<<curr_feature_map_h_<<std::endl;
	int regOffset = curr_feature_map_w_*curr_feature_map_h_; //25668
	// the first count numbers are confidence of face
	int count = confidence->count()/2;// 25993
...
void MTCNN::GenerateBoundingBox(Blob<float>* confidence,Blob<float>* reg,
  ...
  for(int i=0;i<count;i++)
  {
	LOG(INFO)<<*(confidence_data+i)<<", kkkkkkkkkkkkk";
	if(*(confidence_data+i)>=thresh)
	{
	  int y = i / curr_feature_map_w_;
	  int x = i - curr_feature_map_w_ * y;
...
/*
void MTCNN::create_model(const std::string &proto_model_dir, bool isCPU)
{
	#ifdef CPU_ONLY
		...
		caffe::Net<float> *net,double thresh,char netName)
{
	...
	//int n = regressed_pading_.size();
	// load crop_img data to datum 
	for(int i=0;i<numBox;i++)//for(int i=0;i<std::min(numBox,n);i++)
	{
		int pad_top	  = std::abs(regressed_pading_[i].bbox.x1 - regressed_rects[i].bbox.x1);
		int pad_left  = std::abs(regressed_pading_[i].bbox.y1 - regressed_rects[i].bbox.y1);
		int pad_right = std::abs(regressed_pading_[i].bbox.y2 - regressed_rects[i].bbox.y2);
...
bool MTCNN::CvMatToDatumSignalChannel(const cv::Mat& cv_mat, Datum* datum)
{
	...
	else if( 1==channels )
	{
		for (int h = 0; h < datum_height; ++h)
		{
			...
			{
				const float* ptr = cv_mat.ptr<float>(h);
				datum->add_float_data(ptr[w*channels]);
			}
		}
	}
	return true;
...
void MTCNN::Detect(const cv::Mat& image,std::vector<FaceInfo>& faceInfo,int minSize,double* threshold,double factor,caffe::Net<float> *_net1p,caffe::Net<float> *_net2r, caffe::Net<float> *_net3o)
{
	// 2~3ms
	...
	// 11ms main consum
	boost::shared_ptr<MemoryDataLayer<float> > mem_data_layer;
	mem_data_layer = boost::static_pointer_cast<MemoryDataLayer<float> >(_net1p->layers()[0]);
	std::vector<Datum> datum_vector;
	
	for(int i=0;i<factor_count;i++) 
	{	//int i = 0;
		double scale = scales[i]; //0.3
		int ws = std::ceil(height*scale);//input_height;
		int hs = std::ceil(width*scale);//input_width;
		
		// wrap image and normalization using INTER_AREA method
		//cv::resize(sample_single,resized,cv::Size(ws,hs),0,0,cv::INTER_AREA);
		cv::resize(sample_single,resized,cv::Size(ws,hs),0,0,CV_INTER_CUBIC);//cv::INTER_NEAREST);
		//cv::resize(sample_single,resized,cv::Size(input_width,input_height),0,0,cv::INTER_NEAREST);
		
		resized.convertTo(resized, CV_32FC3, 0.0078125,-127.5*0.0078125);
		// input data
		mem_data_layer->addreshape(1,3,hs,ws);
		datum_vector.clear();
		Datum datum;
		CvMatToDatumSignalChannel(resized,&datum);
		datum_vector.push_back(datum);
		
		mem_data_layer->set_batch_size(1);
		mem_data_layer->AddDatumVector(datum_vector);
		float no_use_loss = 0;
		_net1p->Forward(&no_use_loss);
		
		const boost::shared_ptr<Blob<float> > confidence = _net1p->blob_by_name("prob1");
		//const float* confidence_data = confidence->cpu_data();
		const boost::shared_ptr<Blob<float> > reg = _net1p->blob_by_name("conv4-2");
		//const float* reg_data = reg->cpu_data();
	...
	/*
	for (int i=0; i<total_boxes_.size(); i++)
	{
		int x1 = int(total_boxes_[i].bbox.x1);
		int y1 = int(total_boxes_[i].bbox.y1);
		int x2 = int(total_boxes_[i].bbox.x2);
		int y2 = int(total_boxes_[i].bbox.y2);
		cv::rectangle(temp1, cv::Point(x1, y1), cv::Point(x2,y2),cv::Scalar(0,0,255));
	}
	cv::imwrite("pNet.jpg", temp1);*/
	//cv:: waitKey(1000);; 
	...
	if(numBox != 0)
	{
		...
		/// Second stage
		ClassifyFace_MulImage(regressed_rects_,sample_single,_net2r,threshold[1],'r');
		...
		if(numBox != 0)
		{
			ClassifyFace_MulImage(regressed_rects_,sample_single,_net3o,threshold[2],'o');
			regressed_rects_ = BoxRegress(condidate_rects_,3);
			faceInfo = NonMaximumSuppression(regressed_rects_,0.7,'m');
		}
        //Adjust border 2017-11-17 modify by lg
        for(size_t i = 0;i<faceInfo.size();i++)
        {
            if( faceInfo[i].bbox.x1 < 0 )
            {
                faceInfo[i].bbox.x1 = 0;
            }
            if( faceInfo[i].bbox.y1 < 0 )
            {
                faceInfo[i].bbox.y1 = 0;
            }
            if( faceInfo[i].bbox.x2 > height )
            {
                faceInfo[i].bbox.x2 = height - 1;
            }
            if( faceInfo[i].bbox.y2 > width )
            {
                faceInfo[i].bbox.y2 = width - 1;
            }
        }
	}
    LOG(INFO)<<"three stage_boxs.size = "<<faceInfo.size();
	regressed_pading_.clear();
	regressed_rects_.clear();
	condidate_rects_.clear();
}

int MTCNN::Emotion(const cv::Mat& image, std::vector<FaceInfo>& faceInfo, std::vector<std::vector<double>>& vvdEmotionProbs, caffe::Net<float> *_net, const float *mean)
{
	std::vector<cv::Mat> vmImages;
	vvdEmotionProbs.clear();
	std::vector<double> vdEmotionProbs;
    
	crop_emotion(image, faceInfo, vmImages);
	for (size_t i=0; i<vmImages.size(); i++)
	{
		vdEmotionProbs.clear();
		predict_emotion(vmImages[i], _net, mean, vdEmotionProbs);
		vvdEmotionProbs.push_back(vdEmotionProbs);
	}
	
	return 0;
}	

int MTCNN::crop_emotion(const cv::Mat& image, const std::vector<FaceInfo>& faceInfo, std::vector<cv::Mat> &vmImages)
{
	vmImages.clear();
	cv::Mat gray;
	if (3 == image.channels())
	{
		cv::cvtColor(image, gray, CV_BGR2GRAY); // 转为灰度图像  
	}
	...
	for (size_t i=0; i<faceInfo.size(); i++)
	{
		...
		cv::Mat crop_img = gray(cv::Range(x1,x2),cv::Range(y1,y2));
		vmImages.push_back(crop_img);
	}
	
	return 0;
}	

int MTCNN::predict_emotion(const cv::Mat &image, caffe::Net<float> *_net, const float *mean, std::vector<double>& vdEmotionProbs)
{
	boost::shared_ptr<MemoryDataLayer<float> > mem_data_layer;
	mem_data_layer = boost::static_pointer_cast<MemoryDataLayer<float> >(_net->layers()[0]);
	int input_width	 = mem_data_layer->width(); //48
	int input_height = mem_data_layer->height(); //48
	std::vector<Datum> datum_vector;
	cv::Mat resized;
	...
	// wrap image and normalization using INTER_AREA method
	//cv::resize(sample_single,resized,cv::Size(ws,hs),0,0,cv::INTER_AREA);
	cv::resize(image,resized,cv::Size(input_width,input_height),0,0,cv::INTER_CUBIC);//cv::INTER_NEAREST);
	//cv::imwrite("crop.bmp", resized);
    
	resized.convertTo(sample_single,CV_32FC1);
	
	int height = resized.rows;
	int width = resized.cols;
	cv::Mat image_mean(height, width, CV_32FC1, (float*)mean);
	
	for (int h=0;h<height;++h)
	{
		for (int w=0;w<width;++w)
		{
			sample_single.at<float>(h,w) = sample_single.at<float>(h,w) - image_mean.at<float>(h,w); 
		}
	}
	...
	CvMatToDatumSignalChannel(sample_single,&datum);
	datum_vector.push_back(datum);
	...
	results = _net->Forward(&no_use_loss);
	...
	for (int j=0;j<batch_size;j++)
	{
		for (int i=0;i<nclasses;i++)
		{
			...
			//	continue;
			vdEmotionProbs.push_back(prob);
            //vdEmotionCats.push_back(this->_mlmodel.get_hcorresp(i));
		}			
	}
